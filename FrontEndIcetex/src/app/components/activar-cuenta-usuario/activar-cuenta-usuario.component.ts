...
export class ActivarCuentaUsuarioComponent implements OnInit {
  ...
  public onChangeRes(value: any): void {
    this.ObtenerCiudadRes(value);
  }

  private ObtenerCiudadRes(idDepartamento: any): void {
    this.arrCiudadRes = [];
  ...
  private ObtenerCiudad(idDepartamento: any): void {
    this.arrCiudad = [];
    this.basico.ObtenerListaCiudad(idDepartamento).subscribe(data => {
      if (data!.error === 'NO'){
        for (const respuesta of data!.respuesta){
  ...
  private CrearActivacion(): void {
    this.clicked = true;
    this.activaService.ActivarCuenta(this.objActivarFinal).subscribe((data: ActivarCuentaResponse | null ) => {
      this.clicked = false;
      if (data!.error === 'NO') {
        const dialogRef = this.dialog.open(CustomModalComponent,
          { width: '450px',
